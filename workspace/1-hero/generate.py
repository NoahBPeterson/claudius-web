...
ROOT = Path(__file__).resolve().parent      # this stage dir (models.json lives here)
WS = ROOT.parent                            # workspace root (.env + output live here)
load_dotenv(WS / ".env")
...
REFS_DIR = WS / "reference-art"             # drop your own CC0 sticker refs here
REF_FILES = ["head.jpg", "claude_pet_cat.webp", "claude_minecart.webp"]
HERO_DIR = WS / "output" / "hero"
LAYERS_DIR = WS / "output" / "layers"
TIMEOUT = 240
...
def key():
    ...
    if not k or "REPLACE_ME" in k:
        sys.exit("ERROR: set OPENROUTER_API_KEY in workspace/.env first.")
    return k
...
def save(img_bytes, path: Path):
    ...
    rel = path.relative_to(WS) if WS in path.parents else path
    print(f"  saved {rel}  ({len(img_bytes)//1024} KB)")
...
def cmd_isolate(args):
    ...
    print(f"-> {out_dir.relative_to(WS)}/\n")
...
def cmd_posesheet(args):
    ...
    save(imgs[0], WS / "output" / f"posesheet-{hero.stem}.png")
...
def cmd_build_rig(args):
    ...
    out = WS / "output" / "claude-mascot-rigged.html"
    out.write_text(html)
    print(f"Wrote {out.relative_to(WS)}  (canvas {W}x{H})")
